...
\
`fin(fileName)`{ .indent-4 }\
`IF fin.fail()`{ .indent-4 }\
`PUT error`{ .indent-4 }\
\
`numXoverO = 0`{ .indent-4 }\
`FOR row = 0...2`{ .indent-4 }\
`FOR col = 0...2`{ .indent-4 } <span style="border: 2px solid black; padding: 0.4em">Read the 9 symbols from the file</span>\
`READ board[row][col]`{ .indent-4 }\
`IF board[row][col] = X`{ .indent-4 }\
`numXoverO + 1`{ .indent-4 }\
`IF board[row][col] = O`{ .indent-4 }\
`numXoverO - 1`{ .indent-4 } <span style="border: 2px solid black; padding: 0.`4em">Keep track of the number of X's and O's</span>\
\
   xTurn ← (numXoverO == 0)
