...
graph TD
    main --> interact
    main --> clearBoard
    interact --> displayOptions
    interact --> displayBoard
    interact --> readBoard
    interact --> writeBoard
    interact --> editSquare
    interact --> getText

    displayBoard --> convertSymbol
    editSquare --> getCoordinates

    interact -- "board, xTurn" --> clearBoard
    interact -- "board" --> displayBoard
    interact -- "board, xTurn, fileName" --> readBoard
    interact -- "board, xTurn, fileName" --> writeBoard
    interact -- "board, xTurn" --> editSquare
    interact -- "fileName" --> getText
    displayBoard -- "symbol" --> convertSymbol
    editSquare -- "row, col" --> getCoordinates
:::

![Structure chart for Tic-Tac-Toe]("./1.0-Using-Objects_images/1.0-Design-Documents_img_4%20(2).jpeg")
