...
> 
> Original file: `1.3-Structures.pdf`
> 
...
A structure is a mechanism to create a custom data type specifically tailored for the needs of a given program. In other words, we do not have to be limited to the built-in data types provided by the $\mathrm{C}++$ language (such as int, float, char, bool, double, etc.).
