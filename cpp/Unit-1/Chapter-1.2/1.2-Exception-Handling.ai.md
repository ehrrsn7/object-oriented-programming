...
> | Scenario | Technique
> | ---
> | User responds to a prompt with data that is outside<br>the expected range | <input> | Use an assert |
> | The file you are attempting to read from does not exist | <input> | While loop and try again |
> | The code depends on a pointer being initialized, but you want to check it just to make sure.Display an error message and re-prompt |
> | The user enters a file-name that has illegal characters.<br>In a grades program, the user entered $-100 \%$ as an earned grade | <input> | Exit the program immediately |
> 
> ## Technique
> 
> | Throw an exception |
