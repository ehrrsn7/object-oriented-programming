...
> 
> Original file: `1.1-Defensive-Programming.pdf`
> 
...
A buffer is like an array, a block of memory set aside for a task. If the programmer sets aside ten slots of memory, what happens when eleven are used? What happens when the programmer uses memory that is not

A buffer is an array of data to be filled with user input reserved?
...
\#include <cassert>
Since asserts are simply $\mathrm{C}++$ statements (more precisely, they are function calls), they can be put in just about any location in the code. An example assert ensuring an index is not negative would be:
