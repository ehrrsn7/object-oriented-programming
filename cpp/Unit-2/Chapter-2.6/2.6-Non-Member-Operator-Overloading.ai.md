...
> 
> Original file: `2.6-Non-Member-Operator-Overloading.pdf`
> 
...
In other words, operator overloading does not add any power to $\mathrm{C}++$; there is nothing you can do with operator overloading that cannot be done with traditional functions. Instead, operator overloading makes it easier and more convenient for you and others to use your code.
...
The arithmetic operators include operators traditionally used for mathematical operations. There are many arithmetic operators supported in $\mathrm{C}++$ : addition $(+)$, subtraction $(-)$, multiplication $(*)$, division $(/)$, and modulus (\%). Additionally there are the bit-wise operators of bit-wise and (\&), bit-wise or (|), and bitwise xor $(\wedge)$.
...
To represent this in $\mathrm{C}++$, we need two Complex objects and the addition operator:
...
Since the increment and decrement operators are unary operators (only one operand), it seems like there is only need for one parameter: the left-hand-side that is being changed. This parameter must be by-reference. However, $\mathrm{C}++$ requires us to define a second parameter so we can tell the difference between the two.
...
The negative $(-)$ and not (1) operators, like the increment and decrement operators before them, are unary. This means that there is only one operand. In addition to negation and not, there are other unary operators supported in $\mathrm{C}++$ : bitwise not $(\sim)$, the dereference operator $(*)$, and the address-of operator ( $\&$ ).
