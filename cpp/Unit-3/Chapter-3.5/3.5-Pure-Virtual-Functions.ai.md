...
> 
> Original file: `3.5-Pure-Virtual-Functions.pdf`
> 
> Generated: 2025-08-03 16:26:22
...
Recall that virtual functions are function pointers residing in a v-table attached to a class. This means that each function pointer is assigned an address at run-time corresponding to which version of the function is to be executed. In the case of pure virtual functions, there is no function implementation for the v-table to point to. Therefore, the function pointer is given the NULL or zero address. This is why the " $=$ " notation is used both in the UML class diagram and in the $\mathrm{C}++$ class definition.
...
6. Provide the $\mathrm{C}++$ class definition for the Date class and the DateJapan class.
